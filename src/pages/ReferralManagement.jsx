...
const { FiPlus, FiFilter, FiSearch, FiDownload, FiGrid, FiList, FiX, FiEye, FiEdit, FiTrash2, FiMessageSquare, FiCalendar, FiUser, FiPhone, FiMail, FiBarChart3 } = FiIcons;

const ReferralManagement = () => {
  const { user } = useAuth();
  const [activeView, setActiveView] = useState('management'); // 'management' or 'tracking'
  ...
  const handleUpdateReferral = async (formData) => {
    try {
      ...
      setReferrals(prev => prev.map(ref => ref.id === editingReferral.id ? updatedReferral : ref));
    ...
    <div className="min-h-screen bg-dental-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-dental-600">Manage all your patient referrals and track their progress</p>
        </div>

        {/* View Toggle */}
        <div className="flex space-x-1 mb-8 bg-white p-1 rounded-lg shadow-lg max-w-md">
          <button
